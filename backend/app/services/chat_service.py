...
#     return {
#         "type": "attendance",
#         "reply": format_attendance_reply(rows),
#         "data": rows,
#     }
...
def _dispatch(db: Session, resolution: Resolution) -> dict:
    if resolution.kind == "shortcut":
        return _dispatch_shortcut(
            db,
            resolution.shortcut_intent,
            resolution.entities,
        )
    ...
    if plan.action == "lookup":
        advisor = advisor_service.find_advisor_by_name(
            db,
            plan.entity_value,
        )
    ...
    if plan.action == "summary" and plan.level == "team":
        try:
            summary = team_service.get_team_summary(
                db,
                plan.entity_value,
            )
        except NotFoundError:
            return {
                "type": "not_found",
                "reply": f"No team matching '{plan.entity_value}'.",
                "data": None,
            }
    ...
    if plan.action == "summary" and plan.level == "company":
        try:
            summary = company_service.get_company_summary(
                db,
                plan.entity_value,
            )
        except NotFoundError:
            return {
                "type": "not_found",
                "reply": f"No company matching '{plan.entity_value}'.",
                "data": None,
            }
    ...
    if plan.action == "leaderboard":
        ...
        return {
            "type": "leaderboard",
            "reply": format_leaderboard_reply(plan.metric, rows),
            "data": rows,
        }
...
def _dispatch_shortcut(
    ...
    if intent == "attendance_check":
        rows = attendance_service.get_attendance_issues(
            db,
            entities.get("team"),
        )
